/*************************************************
 * Project: Panoptes
 * File: CombatDraftCache.cs
 * Author: Panoptes Team
 * Date: 2026-04-13
 * Description: Server-authoritative combat draft + path preview cache.
 *************************************************/
...
{
    public sealed class CombatDraftCache : MonoBehaviour
    {
        public static CombatDraftCache Instance { get; private set; }

        private readonly Dictionary<string, QueuedCombatOrderDto> _ordersByUnitId = new(StringComparer.OrdinalIgnoreCase);
        ...
        public IReadOnlyDictionary<string, QueuedCombatOrderDto> OrdersByUnitId => _ordersByUnitId;
        ...
        public static CombatDraftCache EnsureInstance()
        {
            if (Instance != null)
            {
                return Instance;
            ...
            var existing = UnityEngine.Object.FindAnyObjectByType<CombatDraftCache>();
            if (existing != null)
            {
                Instance = existing;
                return existing;
            ...
            var go = new GameObject("CombatDraftCache");
            return go.AddComponent<CombatDraftCache>();
        }

        public List<QueuedCombatOrderDto> GetOrdersInDisplayOrder()
        {
        ...
        public void ApplyOrdersSnapshot(MsgCombatOrdersSnapshot msg)
        {
            SnapshotTurn = msg != null ? msg.Turn : 0;
            SnapshotPhase = msg != null ? (msg.Phase ?? string.Empty) : string.Empty;
            _ordersByUnitId.Clear();

            if (msg != null && msg.Orders != null)
            {
                for (var i = 0; i < msg.Orders.Count; i++)
                {
        ...
        public void ApplyPreviewResponse(MsgCombatPathPreviewResponse msg)
        {
            if (msg == null)
            {
                return;
        ...
        private static QueuedCombatOrderDto MapOrder(QueuedCombatOrder order)
        {
            return new QueuedCombatOrderDto
            {
                UnitId = order.UnitId,
                Action = order.Action,
        ...
        private static PathPreviewDto MapPreview(MsgCombatPathPreviewResponse msg)
        {
